				...
				{service.about_service && (
					<div className='w-full h-auto flex flex-col flex-wrap lg:mx-20 md:mx-15 sm:mx-10 mx-5 my-10 gap-y-5 '>
						{service.about_service.map((data, index) => (
							<div
								key={data.id || index}
								className='lg:w-[100%] md:w-[100%] sm:w-[100%] w-full flex lg:flex-row md:flex-row sm:flex-col flex-col items-center lg:gap-x-7 md:gap-x-8 sm:gap-x-5 gap-x-0  text-white'
							>
								<img
									src={data.img}
									alt='about'
									className='lg:w-[60px] md:w-[70px] sm:w-[80px] w-90px'
								/>
								<h2 className='w-full flex justify-center lg:text-3 md:text-2xl sm:text-xl text-xl'>
									{data.header}
								</h2>
								<p className='w-full text-start lg:text-lg md:text-md sm:text-sm text-gray-200/70'>
									{data.text}
								</p>
							</div>
						))}
